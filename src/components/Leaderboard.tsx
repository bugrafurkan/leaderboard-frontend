...
const Leaderboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      try {
        const response = await axios.get('http://34.106.62.0/api/v1/leaderboard');
    ...
    const socket = io('http://34.106.62.0');
          ...
          <Box sx={{ display: 'flex', gap: 2, flex: 1 }}>
            ...
            <TextField
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(id)) {
                    axios.get(`http://34.106.62.0/api/v1/leaderboard?searchPlayerId=${id}`)
                        .then(response => {
